...
const getRoleBadgeClasses = (role?: string) => {
  ...
  if (role === 'parent') return 'bg-brand-100 text-brand-700 dark:bg-brand-500/15 dark:text-brand-300';
  if (role === 'admin') return 'bg-brand-100 text-brand-700 dark:bg-brand-500/15 dark:text-brand-300';
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-4">
        <div>
          <h3 className="text-2xl font-black tracking-tighter text-slate-800 dark:text-white flex items-center gap-3">
            <i className="fas fa-people-arrows text-brand-500"></i> Parent Messaging Desk
          </h3>
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        {[
          { label: 'Parent Contacts', value: parentContactSummaries.length, icon: 'fa-user-group', color: 'text-brand-500 bg-brand-500/10' },
          { label: 'Observed Threads', value: parentConversations.length, icon: 'fa-comments', color: 'text-indigo-500 bg-indigo-500/10' },
          { label: 'Direct Threads', value: directThreadCount, icon: 'fa-paper-plane', color: 'text-brand-500 bg-brand-500/10' },
      ...
      <div className="flex bg-white dark:bg-slate-900 rounded-[28px] border border-slate-100 dark:border-slate-800 shadow-premium overflow-hidden" style={{ height: '700px' }}>
        <div className="w-96 border-r border-slate-100 dark:border-slate-800 flex flex-col shrink-0">
          <div className="p-4 border-b border-slate-100 dark:border-slate-800 space-y-3">
            ...
            <div className="rounded-2xl border border-brand-200 bg-brand-50 dark:border-brand-500/20 dark:bg-brand-500/10 px-4 py-3">
              <p className="text-[10px] font-black uppercase tracking-widest text-brand-700 dark:text-brand-300">Admin Identity</p>
